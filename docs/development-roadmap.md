...
完成 P3 后发布内部开发版本 `0.1.0-dev`，用于日常真实设备试用。
...
1. linear RGB 与 sRGB 转换、量化、缓动函数。
2. 从当前插值位置重定向的交叉渐变。
3. 连续相位呼吸。
4. success 单脉冲和 error 双脉冲。
5. monotonic 调度器、帧去重和“只保留最新帧”背压。
6. 按设备串行提交、失败隔离、退避和降级。
7. asusd 名称所有者监听、重新发现和目标恢复。
8. 睡眠/唤醒以及 daemon 停止时的恢复验证。
...
- 呼吸曲线在周期边界连续，强度不越界。
- 相同量化帧不重复提交。
- 写入慢于 10 Hz 时，没有无界队列，也没有同设备并发写入。
- 瞬态结束后恢复当时最高优先级的持续状态。
- stale session 超时后被回收。
- 单设备连续失败不影响其他设备，重试不会刷屏。
