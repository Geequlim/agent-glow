...
daemon 使用单调时钟驱动 15 Hz 动画，状态切换默认以 300 ms 在线性 RGB 中平滑过渡。
每个非空闲状态都可配置最短展示时间；保护期内只保留仲裁器的最新目标，中间被覆盖的
状态不会排队补播。每台设备最多
只有一个写入进行中，等待区只保留最新量化帧；失败会以 250 ms 到 5 秒的有界指数
退避重试。`success` 使用两次绿色柔和脉冲并回落到低亮 Codex 蓝，`error` 使用
红色双脉冲，状态切换过程中收到新目标时
会从当前可见帧继续过渡。

`success`、`error` 和 `paused` 默认最多保持 10 分钟；一次性动画结束后停留在
低亮终止帧，新活动会立即替换旧结果。最长保持时间可在灯光样式页配置。没有显式
TTL 的其他持续租约会在 5 分钟无更新后作为 stale session 回收。asusd 服务
重启后 backend 会重新发现设备并恢复当前逻辑目标；系统睡眠前恢复硬件快照，唤醒后
重新捕获快照并应用当前状态。

概览页的省电模式默认开启。daemon 通过 Linux power_supply 状态检测供电来源：
使用电池时恢复系统灯光并停止写入动画帧，Agent 事件仍正常记录；接通外部电源后
自动恢复当前最新状态。关闭省电模式后，电池供电时也会正常展示灯效。
